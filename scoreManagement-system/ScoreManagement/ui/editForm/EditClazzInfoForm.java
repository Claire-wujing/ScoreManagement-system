...
import pkg17.ScoreManagement.ui.mListForm.ClazzListForm;
 ...
 *
 */
public class EditClazzInfoForm extends JFrame {
	private JTextField clazzNoField;
	private JButton saveButton, cleanButton;
	private String clazzNo;// 班级号
	private ClazzListForm parentForm;// 父窗体
	 ...
	 */
	public EditClazzInfoForm(ClazzListForm parentForm, String clazzNo) {
	...
	private void initComponents() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent windowEvent) {
				// 刷线父窗体的数据
				parentForm.displayTable(ClazzInfoDao.getClazzList(""));
				// 关闭子窗体
				dispose();
			}
		});
		...
		// 查询初始化数据
		ClazzInfo cInfo = ClazzInfoDao.getClazzInfo(clazzNo);
		...
		clazzNoField.setText(cInfo.getClazzNo());
		...
		saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == saveButton) {
					...
					try {
						ClazzInfoDao.udpateClazzInfo(saveClazzInfo);
