...
public class StudentChangeForm extends JFrame implements ActionListener {
    /**
     *
     */
    private static final long serialVersionUID = 1L;
    JTextField usernameField;
    JPasswordField passwordField, newpasswordField, surepasswordField2;
    JButton sureButton;
    ...
    public StudentChangeForm() {
        ...
        passwordField.setBounds(100, 40, 150, 25);
        passwordField.setText("");

        JLabel newpasswordLabel = new JLabel("新密码:");
        newpasswordField = new JPasswordField();
        newpasswordLabel.setBounds(10, 70, 80, 25);
        newpasswordField.setBounds(100, 70, 150, 25);
        newpasswordField.setText("");

        JLabel surepasswordLabel = new JLabel("确认新密码:");
        surepasswordField2 = new JPasswordField();
        surepasswordLabel.setBounds(10, 100, 80, 25);
        surepasswordField2.setBounds(100, 100, 150, 25);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            if (studentInfo != null) {
                // 判断密码是否正确（这里可以移除，因为不需要验证当前密码）
                // if (studentInfo.getPassword().equals(password)) {
                String sql = "update student set password =? where studentId =?";
                // 进行数据库更新操作
                boolean update = DatabaseUtil.update(sql, newpassword, studentInfo.getStudentId());
                if (update) {
                ...
                // } else {
                //     JOptionPane.showMessageDialog(this, "密码或用户错误", "提示", JOptionPane.WARNING_MESSAGE);
                // }
            } else { // 账号不存在
